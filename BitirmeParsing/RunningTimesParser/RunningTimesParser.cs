...
{
    public class RunningTimesParser : ParserBase<Movie>
    {

        string newTableName;
        string readFromtableName;

        public RunningTimesParser(string readFromtableName, string newTableName_)
        {
        ...
        //    Task.Run(() =>
        //    {
        //        using (FileStream fs = File.Open(ConfigurationSettings.AppSettings["runningTimesListLocation"], FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            ...
            using (StreamReader sr = new StreamReader(bs, System.Text.Encoding.Default))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if (currentMovieName != s)
                    {
                        ...
                        {
                            Console.WriteLine("Running times parser  " + addCounter + "    ");
                            dataItems.Add(bufferList);
                            bufferList = new List<Movie>();
